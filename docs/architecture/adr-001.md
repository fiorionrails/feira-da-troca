...
### Alternativa C: Híbrido com sync eventual (escolhida como arquitetura alvo)
...
!!! warning "Implementação atual"
    A **sync com Firebase não foi implementada**. É uma ideia para o futuro. O campo `synced_to_firebase` existe no banco como preparação, mas nenhum worker de sync foi desenvolvido. O sistema funciona completamente sem Firebase — e assim permanece por enquanto.
